...
const Signup = () => {
  ...
  const submitHandler = async(e) => {
    e.preventDefault();
    try{
      const res = await axios.post("http://127.0.0.1:8080/api/v1/user/register",input,{
        headers:{
  ...
  return (
    <div className='flex items-center justify-center w-screen pt-20'>
        <form onSubmit={submitHandler} className='flex flex-col gap-3 bg-white p-4 w-[20%]'>
            ...
            <button type='submit' className='bg-blue-500 text-white rounded-md p-2 hover:bg-blue-400'>Sign Up</button>
